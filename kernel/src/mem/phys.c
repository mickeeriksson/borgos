...
void phys_pagemap_init(size_t pfn_offset, size_t pfn_count) {
    size_t pagemapsize = pfn_count * sizeof(pageframe_t);
...
void phys_freepages_low(void) {
    ...
    while (p<=PFN_MAX && p<=MAXBOOTPGD_PFN) {
        pageframe_t *page = &pageframemap[p];
...
void phys_init(void) {
    ...
    log_msg("PFN_MAX=%d\nPFN_NORMAL_MIN=%d\nPFN_NORMAL_MAX=%d\n",PFN_MAX,PFN_NORMAL_MIN,PFN_NORMAL_MAX);
    ...
    pageframe_debugprint_free();
    //build the buddy structure
    phys_freepages_low();

    pageframe_debugprint_free();

    pageframe_t* page = pageframe_alloc_pages(GFP_DMA16, 2);

    pageframe_t* page2 = pageframe_alloc_pages(0, 2);
